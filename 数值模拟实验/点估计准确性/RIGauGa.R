  ...
  while(k <= number) {
    ...
    if (!all(is.na(Res))) {
      if (Res$index == 1) {
        ...
        result[[2]][k] <- Res$Mu
        result[[3]][k] <- (2 * Res$Lambda - 1) * (Res$Lambda - 1) / Res$Lambda^2
        ...
        V.tau <- (2 * Res$Lambda - 1) * (Res$Lambda - 1) / Res$Lambda^2
        
        lower.bound[3] <- V.tau - qnorm(0.975) * se[3] * abs((3 * Res$Lambda - 2) / Res$Lambda^3)
        upper.bound[3] <- V.tau + qnorm(0.975) * se[3] * abs((3 * Res$Lambda - 2) / Res$Lambda^3)
