    ...
    for(i in 1:n) {
      ...
      grad_i <- numDeriv::grad(
        func = function(th) {
          ...
          lambda <- th[3]
          # 计算单个观测的对数似然
          term1 <- gamma(alpha + lambda) * alpha^alpha * (lambda - 1)^lambda * x[i]^(alpha - 1)
          term2 <- mu^alpha * gamma(alpha) * gamma(lambda)
          term3 <- (alpha * x[i] / mu + lambda - 1)^(-alpha - lambda)
          log_val <- log(term1 / term2 * term3)
          return(log_val)
        },
        theta,
        method = "Richardson",
  ...
  while(k <= number) {
    ...
    if (!all(is.na(Res))) {
      if (Res$index == 1) {
        ...
        lower.bound[3] <- 1 / (Res$Lambda - 2) - qnorm(0.975) * se[3] / (Res$Lambda - 2)^2
        upper.bound[3] <- 1 / (Res$Lambda - 2) + qnorm(0.975) * se[3] / (Res$Lambda - 2)^2
